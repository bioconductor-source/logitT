...
After invoking the \textsl{logitTAffy} function, the result is a named vector containing the t-statistics for each probe set for each array. 
...
We thank Sandy Liyanarachchi for providing the original C++ implementation of the Logit-t algorithm as decribed by Dr William J. Lemon, Dr Ming You, and Dr Sandya Liyanarachchi. This work was partially supported by National Institute of Diabetes and Digestive and Kidney Diseases R01DK069859.
...
\bibitem{R2008} R Development Core Team (2008), R: A Language and Environment for Statistical Computing Version 2.7.1. R
